...
from versa.utterance_metrics.dpam_distance import DpamDistanceMetric
...
def test_utterance_dpam_distance(use_gpu, fixed_audio, fixed_ground_truth):
    """
    Test the DPAM distance metric using the fixed audio files.
    ...
    assert "dpam_distance" in result, "Result should contain 'dpam_distance' key"

    # Check that the result is a float
    dpam_dist = result["dpam_distance"]
    assert isinstance(dpam_dist, float), "dpam_distance should be a float"

    # Check that the distance score is reasonable (should be non-negative)
    assert dpam_dist >= 0.0, f"DPAM distance should be non-negative, got {dpam_dist}"


def test_dpam_distance_metric_metadata():
    """Test that the DPAM distance metric has correct metadata."""
    config = {"use_gpu": False}
    metric = DpamDistanceMetric(config)
    metadata = metric.get_metadata()

    assert metadata.name == "dpam_distance"
    ...
    assert "filelock" in metadata.dependencies


def test_dpam_distance_metric_different_sample_rates():
    """Test that the DPAM distance metric handles different sample rates correctly."""
    config = {"use_gpu": False}
    metric = DpamDistanceMetric(config)
    ...
    assert (
        "dpam_distance" in result_44k
    ), "44kHz result should contain 'dpam_distance' key"
    assert (
        "dpam_distance" in result_22k
    ), "22kHz result should contain 'dpam_distance' key"

    # Both should return float values
    assert isinstance(result_44k["dpam_distance"], float)
    assert isinstance(result_22k["dpam_distance"], float)


def test_dpam_distance_metric_invalid_input():
    """Test that the DPAM distance metric handles invalid inputs correctly."""
    config = {"use_gpu": False}
    metric = DpamDistanceMetric(config)
...
def test_dpam_distance_metric_config_options():
    """Test that the DPAM distance metric handles different configuration options."""
    # Test with GPU disabled
    config_cpu = {"use_gpu": False}
    metric_cpu = DpamDistanceMetric(config_cpu)

    # Test with custom cache directory
    config_custom_cache = {"use_gpu": False, "cache_dir": "custom_cache"}
    metric_custom_cache = DpamDistanceMetric(config_custom_cache)
    ...
    result_custom_cache = metric_custom_cache.compute(audio1, audio2, metadata=metadata)

    # All should return the same structure
    assert "dpam_distance" in result_cpu
    assert "dpam_distance" in result_custom_cache
    assert isinstance(result_cpu["dpam_distance"], float)
    assert isinstance(result_custom_cache["dpam_distance"], float)


def test_dpam_distance_metric_identical_signals():
    """Test that the DPAM distance metric gives zero distance for identical signals."""
    config = {"use_gpu": False}
    metric = DpamDistanceMetric(config)
    metadata = {"sample_rate": 22050}
    ...
    assert result["dpam_distance"] == 0.0, "Identical signals should have zero distance"


def test_dpam_distance_metric_consistent_results():
    """Test that the DPAM distance metric gives consistent results for the same inputs."""
    config = {"use_gpu": False}
    metric = DpamDistanceMetric(config)
    metadata = {"sample_rate": 22050}
    ...
    np.testing.assert_almost_equal(
        result1["dpam_distance"],
        result2["dpam_distance"],
        decimal=6,
