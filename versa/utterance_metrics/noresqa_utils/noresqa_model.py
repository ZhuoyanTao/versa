...
class how_snr(nn.Module):
    def __init__(self, dim_emb=32, output=50):
        ...
        self.dp.append(nn.Dropout(p=dp_num))
        self.encoder.append(
            nn.Conv1d(32, output, filter_size, padding=filter_size // 2)
        )
    ...
    def __init__(
        ...
        if metric_type == 0:
            ...
            self.base_encoder_2 = TemporalConvNet(
                num_inputs=128, num_channels=[32, 64, 128, 64], kernel_size=3
            )
            ...
            SSL_OUT_DIM = 768
            ssl_model, _, _ = fairseq.checkpoint_utils.load_model_ensemble_and_task(
                [config_path]
            )
